...
- **The PR build must be in Atlas first.** A build only has Atlas data once Revyl has observed it. Otherwise the head shows 0 screens, the verdict is "Atlas data missing", and the check goes **neutral** with a message to map it. `--wait-for-map` bridges the lag when mapping runs but lands a little after the upload; it cannot start mapping on its own. There are two ways to feed Atlas, and they catch different things:
  - **Full exploration** of the PR build (app-explorer / production trigger) is the only path that surfaces net-new **untested** screens — the headline signal. There is no `revyl` CLI for it; it is an upstream step.
  - **Run your test suite against the build** (`revyl workflow run <suite> --build`) is a CLI-accessible lighter feeder: test reports feed Atlas, so it maps what your tests cover (good for changed/removed/coverage-drift), but it will not show screens your tests never reach.
